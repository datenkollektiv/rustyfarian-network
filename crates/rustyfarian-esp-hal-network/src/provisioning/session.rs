...
pub enum ProvisioningError {
    ...
    ///
    /// v1 of `rustyfarian-esp-hal-network` only implements the `WifiMqttDevice`
    /// profile.
...
impl<'a> ProvisioningBuilder<'a> {
    ...
    /// - `ap` — [`SoftApHandle`](crate::wifi::SoftApHandle) from
...
pub(crate) trait PortalStore: Send + Sync {
    ...
    fn load(
        &self,
    ) -> Result<Option<juggler::provisioning::ProvisioningConfig>, super::store::StoreError>;
}
...
pub(crate) mod tasks {
    ...
    use super::super::dhcp::{run as dhcp_run, DhcpServerConfig};
    use super::super::dns_catchall::{run as dns_run, DnsCatchallConfig};
    use super::super::portal::run_portal_dyn;
