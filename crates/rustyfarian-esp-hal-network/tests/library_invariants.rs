...
//!
//! ```sh
//! cargo test --test library_invariants -p rustyfarian-esp-hal-network --no-default-features
//! ```
//!
//! # Security-checklist item 7
//!
//! The provisioning library must never call `esp_hal::reset`, `software_reset`,
//! or `store.erase_all()` from any portal handler — those are integrator
//! decisions.  The `library_does_not_call_esp_hal_reset` test enforces this
//! by grepping the provisioning module source and asserting zero matches.

use std::process::Command;

/// Security-checklist item 7 lock: the provisioning library source must never
/// call `esp_hal::reset`, `software_reset`, or `esp_hal_reset`.
///
...
///
/// This test spawns `rg` (ripgrep) to search the provisioning module source.
...
fn library_does_not_call_esp_hal_reset() {
    let crate_src = concat!(env!("CARGO_MANIFEST_DIR"), "/src/provisioning");
...
fn portal_handlers_do_not_call_erase_all() {
    ...
    let portal_src = concat!(env!("CARGO_MANIFEST_DIR"), "/src/provisioning/portal.rs");
