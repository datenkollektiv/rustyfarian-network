...
use crate::mqtt::{validate_client_id, CLIENT_ID_MAX_LEN};

#[cfg(test)]
use crate::provisioning::config::MAX_FIELD_ERRORS;
...
use crate::provisioning::error::{Field, FieldError, FieldErrors, ValidationError};
...
/// guarantees.
// `FieldErrors` is a heapless aggregate of per-field errors; in this `no_std`
// crate there is no allocator to box it, so the large `Err` variant is
// intentional (same rationale as the `clippy::large_enum_variant` allow on
// `LorawanResponse`). Only fires on 64-bit hosts; harmless on the 32-bit target.
...
pub fn parse_form(body: &str, profile: SchemaProfile) -> Result<ProvisioningConfig, FieldErrors> {
    ...
    let mut wifi_ssid: heapless::String<{ crate::wifi::SSID_MAX_LEN }> = heapless::String::new();
    let mut wifi_password: heapless::String<{ crate::wifi::PASSWORD_MAX_LEN }> =
        heapless::String::new();
...
fn validate_wifi_ssid(
    slot: &Slot,
    out: &mut heapless::String<{ crate::wifi::SSID_MAX_LEN }>,
    ...
    if slot.overflowed || slot.value.len() > crate::wifi::SSID_MAX_LEN {
        push_field_error(
            errors,
            Field::WifiSsid,
            ValidationError::TooLong {
                max: crate::wifi::SSID_MAX_LEN,
            },
        );
        return;
    }
    if crate::wifi::validate_ssid(&slot.value).is_err() {
...
fn validate_wifi_password(
    slot: &Slot,
    out: &mut heapless::String<{ crate::wifi::PASSWORD_MAX_LEN }>,
    errors: &mut FieldErrors,
) {
    if slot.duplicate {
    ...
    if slot.overflowed || slot.value.len() > crate::wifi::PASSWORD_MAX_LEN {
        push_field_error(
            errors,
            Field::WifiPassword,
            ValidationError::TooLong {
                max: crate::wifi::PASSWORD_MAX_LEN,
            },
        );
        return;
    }
    ...
    if !slot.value.is_empty() && slot.value.len() < crate::wifi::AP_PASSWORD_MIN_LEN {
        push_field_error(
            errors,
            Field::WifiPassword,
            ValidationError::TooShort {
                min: crate::wifi::AP_PASSWORD_MIN_LEN,
            },
        );
        return;
    }
    if crate::wifi::validate_password(&slot.value).is_err() {
        push_field_error(
            errors,
            Field::WifiPassword,
            ValidationError::TooLong {
...
/// [`validate_client_id`](crate::mqtt::validate_client_id),
