...
**ACK mechanism**: `esp-idf-svc::espnow::EspNow::send()` is synchronous and returns `Ok(())` only on MAC-layer ACK.
No raw C callback registration is needed.
...
1. Register peer with `channel=0` (follows current channel)
2. For each channel in `ScanConfig::channels`:
   - `esp_wifi_set_channel(ch, WIFI_SECOND_CHAN_NONE)`
   - `send(mac, probe_data)`
   - On ACK: re-register peer with discovered channel, return `Ok(ScanResult)`
3. On exhaustion: clean up peer registration, return `Err`
...
- No protocol changes needed (MAC-layer ACK is the probe mechanism)
...
- Worst-case scan latency is ~1.3 s (13 channels x ~100 ms send timeout)
...
- Feature request: `review-queue/espnow-channel-scanning.md`
