...
Proposed — awaiting maintainer sign-off before implementation begins.
...
|:----------------------|:-----------------------------------------------------------------------------------------------------|:---------|:-------------------------------------------------------------------------------|
| `rustyfarian-pure`    | `rustyfarian-network-pure`, `wifi-pure`, `lora-pure`, `espnow-pure`, `ota-pure`, `provisioning-pure` | `no_std` | `wifi`, `mqtt`, `lora`, `espnow`, `ota`, `provisioning`, `mock`                |
...
- `rustyfarian-pure 0.3.0` is published to crates.io with version `0.3.0` (incremented from the current `0.2.1`).
- `rustyfarian-esp-idf 0.3.0` adds a workspace dependency on `rustyfarian-pure = "0.3"` (semver-compatible).
- `rustyfarian-esp-hal 0.3.0` adds a workspace dependency on `rustyfarian-pure = "0.3"`.
...
After publication, downstream consumers (outside the workspace) depend on `rustyfarian-esp-idf` or `rustyfarian-esp-hal` directly; they do not directly depend on `rustyfarian-pure` (though it remains a transitive dependency and is publicly re-exported by the HAL crates for consumers who need the shared types).
...
rustyfarian-esp-idf = { version = "0.3", features = ["wifi", "mqtt"] }
...
rustyfarian-esp-hal = { version = "0.3", features = ["wifi", "ota", "provisioning", "esp32c6", "embassy"] }
```

### 4. Pure-tier exports the consolidated pure crates; HAL crates re-export the domains they support

The consolidated `rustyfarian-pure` crate uses module-level re-exports to surface each domain's public types and traits:
...
**`rustyfarian-pure`:**
- `no_std`, zero hardware or runtime dependencies.
- Contains shared types, traits, protocol logic, state machines, and validation only.
...
**`rustyfarian-esp-idf`:**
- `std`, ESP-IDF only.
- Depends on `rustyfarian-pure` (selected features) plus `esp-idf-svc`, `esp-idf-hal`, and related ecosystem crates.
...
**`rustyfarian-esp-hal`:**
- `no_std` bare-metal.
- Depends on `rustyfarian-pure` (selected features) plus `esp-hal` (with `unstable` feature as needed), `esp-radio`, and bare-metal ecosystem crates.
...
- One-way: both `rustyfarian-esp-idf` and `rustyfarian-esp-hal` depend on `rustyfarian-pure`, never the reverse.
...
- The workspace has never been published to crates.io (git-tag-only to date), so only internal-facing projects (first-party firmware repos using git deps) are affected.
- Adding ~16 thin compatibility crates directly opposes the consolidation goal and adds indefinite maintenance burden.
- A coordinated breaking change with a clear migration guide plus a single semver bump (0.2.1 → 0.3.0) is the pre-1.0 cost of making a architectural decision.

**Note for the maintainer:** this is a recommendation, not a lock; the final decision rests with the maintainer.

### 8. Semver impact: BREAKING change, version 0.3.0

This consolidation is a **major breaking change** that affects every downstream consumer.
The version bump is `0.2.1` → `0.3.0` (or higher if other breaking changes land in the same release cycle).
...
- **The crate names are unambiguous:** `rustyfarian-esp-idf`, `rustyfarian-esp-hal`, and `rustyfarian-pure` are self-documenting; there is no naming conflict or ambiguity about which tier to use.
...
- **Reduced publication ceremony:** publishing `rustyfarian-esp-idf 0.3.0` publishes all six domains at once with a single `cargo publish` and a single CHANGELOG entry, instead of staggering six separate publishes and coordinating pre-release ordering.
- **Cleaner crates.io namespace:** `rustyfarian-pure`, `rustyfarian-esp-idf`, and `rustyfarian-esp-hal` are the three user-facing crates; the six per-domain crates become internal implementation details not visible on crates.io.
...
- **Breaking rename for downstream:** any external crate already depending on (e.g.) `rustyfarian-esp-idf-wifi` will need to migrate to `rustyfarian-esp-idf` with the `wifi` feature.
  This is acceptable as a one-time cost at initial publication (the workspace is not yet on crates.io), but it should be called out in the release notes and the migration guide.
...
- A per-crate README section for `rustyfarian-pure`, `rustyfarian-esp-idf`, and `rustyfarian-esp-hal` covering the feature tables, chip support, and usage examples.
