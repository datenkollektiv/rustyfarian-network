...
- [x] On-hardware captive-portal smoke test (ESP32-C3, phone browser) — validated 2026-06-18 on an ESP32-C3 Super Mini (see Session Log)
...
- 2026-06-18 — **On-hardware captive-portal smoke test PASSED** on an ESP32-C3 Super Mini (`hal_c3_provision_mqtt`).
  Full happy path validated live: SoftAP up (open-AP warning fired), AP IP 192.168.4.1, DHCP lease from the pool, DNS catch-all, Android `generate_204` captive detection, full portal `GET /` render (no MINIMAL_500 — confirms `DEFAULT_TX_BUF = 6144`), nonce-guarded `POST /save` accepted, credentials committed to flash, and persistence verified across a reboot (`is_provisioned` → `load` round-trips SSID / device name / profile / MQTT host+port). Serial logging confirmed lengths-only (no credential bytes); no stack overflow at the ~13 KiB POST peak.
  The run surfaced two hardware-only bugs, both fixed in this pass:
  (1) `ProvisioningStore::open` bounds check treated `total_bytes` as an absolute extent from offset 0 rather than the partition-region size, panicking `OffsetOutOfBounds` at the real 3 MiB offset — host tests had only ever used `base_offset` 0/4096. Fixed to check `base_offset + total_bytes <= capacity`; new regression test `store_open_accepts_region_at_high_offset_within_capacity`; doc + `StoreError::OffsetOutOfBounds` doc corrected.
  (2) The example's already-provisioned idle path called `Timer::after().await`, but the esp-rtos async executor accesses the scheduler's time driver on *any* `.await` park, and `esp_rtos::start` is only invoked inside `WiFiManager::init_async`/`init_softap_async`; the no-Wi-Fi provisioned path therefore panicked `time_driver NoneError`. Replaced the async idle with a non-async spin halt in both `hal_c3_provision_mqtt` and `hal_c6_provision_mqtt`.
  Not exercised on hardware (acceptable — covered by host tests or out of v1 scope): factory-reset path, live nonce-403 / body-413 negative checks, ESP32-C6.
  Observed minor discrepancy (tracked separately, not blocking): the AP SSID logged `len=11` = bare `RustyFarian` with no MAC suffix despite the example doc claiming the last two MAC bytes are appended.
