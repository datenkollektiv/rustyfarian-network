...
HAL and IDF builds can run concurrently without conflict because they write to distinct target
...
- [ ] Concurrent HAL and IDF builds complete without conflict
- [ ] Host-only workflows (`just verify`, `just test`) remain unchanged and use `target/ide`
- [ ] Builds without RAM disk still land in `target/hal` and `target/idf` (not `target/`)
- [ ] `clean-idf` removes only IDF-specific build artefacts; does not touch `target/hal`
- [ ] `just ramdisk attach` is idempotent; re-running when already attached is safe
- [ ] PARTIAL state (volume mounted, subdirs missing) is recovered by re-running `just ramdisk attach`
