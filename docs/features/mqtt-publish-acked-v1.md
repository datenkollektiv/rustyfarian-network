...
- [ ] **`MessageId` id-space verification** — *promoted to a blocking pre-implementation prerequisite* (see the `MessageId` correlation Constraint and the first State item). Resolve by confirming against the installed `esp-idf-svc` that `enqueue`'s returned id and `EventPayload::Published`'s id are the same space; if they diverge, key the registry off `enqueue`'s return and match it to the event value. Left here as a tracked checkbox until verified.
...
- [ ] **Where the event-loop `ThreadId` is captured and stored** so the `WrongThread` guard can read it — on `MqttHandle` (set at `build()`), since the handle is what `publish_acked` is called on.

## State

- [x] Design approved
- [ ] **Prerequisite verified** — `enqueue`'s returned `MessageId` and `EventPayload::Published`'s id are confirmed to be the same id space on the installed `esp-idf-svc` (see Constraints). The correlation design is unsound until this holds; do not start core implementation before it is checked.
- [ ] Core implementation — pure registry in `juggler::mqtt`; `publish_acked` + `PublishAckError` + event-loop `Published`/`Disconnected` wiring + `WrongThread` guard in `rustyfarian-esp-idf-network::mqtt`
- [ ] Tests passing — host tests for the pure correlation logic (register/resolve/fail_all/timeout); `just verify` green
- [ ] Documentation updated — module docs + method rustdoc (with the "forbidden from callbacks" note) and CHANGELOG `[Unreleased] → Added`
